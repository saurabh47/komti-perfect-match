...
export class AppComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.route.queryParamMap.subscribe((params) => {
            ...
            if (sessionId) {
                forkJoin({
                    session: this.usersService.getSessionDetails(
                    ...
                    this.profileFiltersStore.setAnnualIncomes(
                        resp.annualIncomes
                    );
                    this.profileFiltersStore.setEducationAreas(
                        resp.educationAreas
                    );

                    this.selectedGender = resp.session.selectedGender;
                    this.selectedProfileFilters =
                        resp.session.profileFilters ??
                        this.selectedProfileFilters;
                });
            }
        });
